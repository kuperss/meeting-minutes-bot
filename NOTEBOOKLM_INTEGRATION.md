...
# Windows PowerShell
cd 專案根目錄
.\.venv\Scripts\Activate.ps1   # 或自己的 venv

# 0-1. 套件裝了嗎?
python -c "import notebooklm, sys; print(f'notebooklm-py {notebooklm.__version__ if hasattr(notebooklm,\"__version__\") else \"installed\"} @ {notebooklm.__file__}')"

# 0-2. CLI 裝了嗎?
notebooklm --version

# 0-3. cookie 存在且有效嗎?(--test 才會打網路驗證)
notebooklm auth check --test

# 0-4. 我們繞過 bug 用的 manual_login.py 在嗎?
ls manual_login.py

# 0-5. 主流程串接 service 在嗎?
ls notebook_service.py prompts.py
```
...
| 檢查項 | ✓ 結果 | ✗ 結果 → 怎麼修 |
|---|---|---|
| 0-1 套件 | 印出版本與路徑 | `pip install notebooklm-py` |
| 0-2 CLI | 印版本 | 同上(CLI 跟套件一起裝) |
| 0-3 cookie | 五項全 pass | 跑 `python manual_login.py`(**不是** `notebooklm login`,理由見 §3-1) |
| 0-4 manual_login.py | 看到檔案 | 從 repo 抄一份(下面 §3-2 有完整內容) |
| 0-5 服務檔 | 看到檔案 | 從本 repo 取(<https://github.com/kuperss/meeting-minutes-bot>) |

**關鍵觀念**(避免新 agent 誤解):
- ✅ **要用 pip 裝 `notebooklm-py`** — 整個整合的核心就是這個套件
- ❌ **不要 git clone teng-lin/notebooklm-py** — 它已經發到 PyPI,clone source 反而麻煩
- ✅ **本專案有寫 `manual_login.py` 繞過套件 CLI 的 bug** — 我們**沒有改套件原始碼**,而是不用它的 `notebooklm login` 指令,自己用 playwright 重做一次同樣動作但跳過會壞的那行
- ✅ **新 agent 該做的:檢查清單跑一遍,缺什麼補什麼**
...
> ⚠️ **正確安裝方式:`pip install notebooklm-py`(從 PyPI),不是 git clone**
>
> - ✅ **要安裝**:這個套件是整合的核心,沒裝就什麼都做不到
> - ❌ **不用 clone source code**:它是發行的 PyPI 套件,`pip install` 完整功能都有
> - 📂 **想看原始碼**:pip 裝完後在 `.venv/Lib/site-packages/notebooklm/` 內(例如要 debug 或對照本文件提到的 bug 位置)
...
### 3-2. 解法:本專案寫了 `manual_login.py`(不是修補套件)

> 🔑 **重要釐清**(避免新 agent 誤解):
> - 我們**沒有改 `notebooklm-py` 套件的原始碼**(那是讀套件的 source 不是 patch)
> - 我們**寫了一支獨立腳本** `manual_login.py`,直接用 playwright 重做「啟動瀏覽器→等使用者登入→存 cookie」的流程
> - 這支腳本**跳過套件 CLI 那行壞掉的 `goto(accounts.google.com)`**
> - 產出的 `storage_state.json` 跟 `notebooklm login` 應該產的是**同一個格式**,所以後面所有 `notebooklm-py` API / CLI 都讀得到、能正常運作
> - 等套件未來修好那個 bug,可以直接改回用 `notebooklm login`,本專案其他 code 都不用動

**完整 `manual_login.py`**(直接複製整支貼到專案根目錄即可,沒有外部相依除了 playwright):
...
> 💡 **新環境/新 agent**:如果這支不在工作目錄,複製上面整段存成 `manual_login.py`,然後 `pip install playwright` + `playwright install chromium` + `python manual_login.py` 即可。**沒有任何專案相依,可獨立使用。**
